...
export type Repo = {
  handle: string;
  granted: string[];
  counts: { collection: string; records: number; more: boolean }[];
};
...
export async function read(session: OAuthSession): Promise<Repo> {
  ...
  const [token, described] = await Promise.all([
    ...
    query<Described>(session, "com.atproto.repo.describeRepo", { repo }),
  ]);
  ...
  // One page each: a panel reporting sizes should not walk a whole repo.
  const counts = await Promise.all(
    present.map(async (collection) => {
      const page = await query<Listed>(
        session,
